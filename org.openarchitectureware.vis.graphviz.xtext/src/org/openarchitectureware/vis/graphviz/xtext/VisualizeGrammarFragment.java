...
/**
 * A Xtext generator fragment to transform the grammar into a graphviz dot file and
 * generate pictures by executing graphviz.
 *
...
public class VisualizeGrammarFragment extends DefaultGeneratorFragment
{
   ...
   private boolean createSeparateRuleGraphs = false;
   ...
   public void generate(Grammar grammar, XpandExecutionContext ctx)
   {
      ...
      String targetDir = ctx.getOutput().getOutlet(Generator.SRC_GEN).getPath() + "/"
            + naming.asPath(GrammarUtil.getNamespace(grammar))+ "/" + imageSubdirectory ;
      ...
      parameters.put("createSeparateRuleGraphs", String.valueOf(createSeparateRuleGraphs));
      parameters.put("inputSlot", "model");
      if(outputFormat != null)
         parameters.put("outputFormat", outputFormat);
      mweRunner.getWorkflowContext().put("model", grammar);
      mweRunner.run("org.openarchitectureware.vis.graphviz.xtext.VisualizeGrammar",
            parameters);
   }
   ...
   public void setCreateSeparateRuleGraphs(boolean createSeparateRuleGraphs)
   {
      this.createSeparateRuleGraphs = createSeparateRuleGraphs;
   }
