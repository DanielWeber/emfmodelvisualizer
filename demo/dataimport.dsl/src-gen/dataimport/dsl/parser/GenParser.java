...
public abstract class GenParser extends AbstractXtextParser implements IXtextParser {
	...
	private impParser parser;

	public GenParser(InputStream in) {
		this.reader = new InputStreamReader(in);
	}

	public impLexer getLexer() {
		if (lexer == null) {
			try {
				this.lexer = new impLexer(new ANTLRReaderStream(reader));
			} catch (IOException e) {
				handleError(e);
			}
		}
	...
	public impParser getParser() {
		if (parser == null) {
			parser = new impParser(new CommonTokenStream(getLexer()));
		}
		return parser;
	}
	...
	protected void internalLink() {
		...
		try {
			...
			XtendFacade facade = XtendFacade.create(ctx, "dataimport::dsl::Linking");
	...
	public Issues doCheck() {
		...
		try {
			...
			String[] checkFiles = new String[]{"dataimport::dsl::GenChecks","dataimport::dsl::Checks"};
