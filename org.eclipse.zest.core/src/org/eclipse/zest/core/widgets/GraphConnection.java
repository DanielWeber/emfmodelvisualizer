...
public class GraphConnection extends GraphItem {
	...
	private double labelLocation = 0.5;
	...
	/**
	 * Sets the position of the label on the connection.  This can be any value between 0..1
	 * @param labelLocation The location of the label
	 */
	public void setLableLocation(double labelLocation) {
	 ...
	 */
	public double getLableLocation() {
	...
	public void setSourceDecoration(PolygonDecoration decoration) {
		this.connectionFigure.setSourceDecoration(decoration);
	}

	public void setTargetDecoration(PolygonDecoration decoration) {
		this.connectionFigure.setTargetDecoration(decoration);
	}
	...
	private PolylineConnection doCreateFigure() {
		PolylineConnection connectionFigure = null;
		...
		if (getSource() == getDestination()) {
			...
			labelLocator = new MidpointLocator(connectionFigure, 0) {
				protected Point getReferencePoint() {
					...
					p.x = (int) ((p2.x - p1.x) * labelLocation) + p1.x;
					p.y = (int) ((p2.y - p1.y) * labelLocation) + p1.y;
					return p;
				}
			};
		}
